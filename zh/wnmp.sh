...
download() {
  ...
  if [[ -z "$enable_public" ]]; then
    read -rp "是否开启 公开目录？[y/N]（Y=开启，N=关闭） " ans
    ans="${ans:-N}"
    if [[ "$ans" =~ ^[Yy]$ ]]; then
      enable_public=1
    else
  ...
  if [[ ! -f "$conf_path" ]]; then
    echo "[download][ERROR] 未找到配置：$VHOST_DIR/${domain_lc}.conf 或 ${domain_lc#www.}.conf"
    return 1
  fi
  ...
  if [[ "$enable_public" -eq 1 ]]; then
    ...
    echo "[download] 已移除 include enable-php.conf;（禁止 PHP 执行）"

    insert_once "$conf_path" "include download.conf;"
    echo "[download] 已确保 include download.conf;"
  else
    sed -i '/^[[:space:]]*include[[:space:]]\+download\.conf;[[:space:]]*$/d' "$conf_path"
    echo "[download] 已移除 include download.conf;"

    insert_once "$conf_path" "include enable-php.conf;"
    echo "[download] 已确保 include enable-php.conf;"
  fi
...
vhost_del() {
  if ! (echo $BASH_VERSION >/dev/null 2>&1); then
    echo "[vhost][ERROR] 请使用 bash 运行此脚本。"; return 1
  fi
  set -euo pipefail
  ...
  if [[ -z "$conf_path" && ! -e "$site_root" && ${#ssl_paths[@]} -eq 0 ]]; then
    echo "[vhost][ERROR] 未找到该域名对应的站点数据: $domain_lc"
    return 1
  fi

  echo "[vhost][WARN] 即将删除以下站点数据:"
  [[ -n "$conf_path" ]] && echo "  Nginx 配置: $conf_path"
  [[ -e "$site_root" ]] && echo "  站点目录:   $site_root"
  for candidate in "${ssl_paths[@]}"; do
    echo "  SSL 目录:   $candidate"
  done

  local ans backup_dir
  read -rp "删除前是否备份站点文件和配置？[Y/n] " ans
  ans="${ans:-Y}"
  if [[ "$ans" =~ ^[Yy]$ ]]; then
    ...
    echo "[vhost][BACKUP] 已备份到: $backup_dir"
  else
    echo "[vhost][WARN] 已选择不备份，开始删除。"
  fi
  ...
  if /usr/local/nginx/sbin/nginx -t; then
    ...
    echo "[vhost] Nginx 已重启。"
  else
    echo "[vhost][ERROR] 删除后 nginx 配置检查失败。"
    return 1
  fi

  echo "[vhost] 已删除: $domain_lc"
}
...
backup_nginx_config() {
  ...
  echo "[backup] Nginx 配置备份已保存：${backup_dir}"
...
backup_php_config() {
  ...
  echo "[backup] PHP 配置备份已保存：${backup_dir}/php.ini"
...
wnmp_restore_nginx_backup() {
  ...
  echo "${label} 已恢复原 Nginx 配置：${backup_dir}"
}
...
wnmp_restore_php_ini_backup() {
  ...
  echo "${label} 已恢复原 php.ini：${php_ini_backup}"
}
...
wnmp_update_nginx() {
  ...
  git --version >/dev/null || { log "缺少 git"; exit 1; }
...
EOF
  ...
  php -v
  echo "[update] PHP 升级完成。"
}

wnmp_update_prepare_proxy() {
  echo "[update] 更新前选择代理隧道..."
  enable_proxy

  if [[ "${PROXY_MODE:-}" != "DIRECT" ]]; then
    if ! proxy_healthcheck; then
      echo "[update][ERROR] 代理隧道健康检查失败。"
      return 1
    fi
  fi
}
...
<body>
  <div class="wrap">
    <div class="topbar">
      ...
      <div class="actions">
        <button class="btn" id="btnReload"><span class="dot"></span>刷新</button>
        <button class="btn" id="btnCopyPath">复制当前路径</button>
      </div>
    </div>
    <div class="copyright"><small>本服务器由 <a href="https://wnmp.org" target="_blank" rel="noopener">wnmp.org</a> 一键包构建 </small></div>
    <div class="grid">
      <div class="panel">
        <div class="panel-hd">
          ...
          <div class="search">
            ...
            <input id="q" placeholder="搜索文件名…（支持模糊匹配）" />
          </div>
        </div>
...
(() => {
  ...
  async function load(){
    ...
    try{
      ...
      tbody.innerHTML = `<tr><td colspan="4" class="empty">加载失败：${esc(e.message || e)}<br><span class="muted">请检查 Nginx 是否已开启 /api/list/ 的 autoindex_format json</span></td></tr>`;
      return;
    }
  ...
  function render(list, keyword){
    ...
    if (!filtered.length){
      tbody.innerHTML = `<tr><td colspan="4" class="empty">空目录 / 没有匹配项</td></tr>`;
      return;
    }
    ...
    const rows = filtered.map(it => {
      ...
      return `
        <tr>
          ...
          <td class="muted last">${esc(mtime)}<button class="op" data-copy="${esc(location.origin + href)}">复制</button></td>
...
<body>
  <div class="box">
    ...
    <p style="font-size:0.9rem;opacity:0.7;">该服务器使用<a style="color:#555" href="https://www.wnmp.org" target="_blank">wnmp.org</a>的一键安装包搭建而成。</p>
  </div>
</body>
</html>
...
if [ "$mariadb_version" != "0" ]; then
  ...
  # WNMP 不使用 ColumnStore，且其旧版 CMake 文件会在 CMake 4 中失败。
  cmake .. \
    -DCMAKE_BUILD_TYPE=Release \
