...
v1.54 2026-08-27 修复精简系统写入 PATH 配置时可能导致安装退出的问题；新增 Nginx、PHP、MariaDB 独立安装，并收纳组件删除/升级菜单。反向代理现可通过 Webroot HTTP-01 申请 SSL 证书，已部署证书会纳入 SSL 管理和续签。

v1.53 2026-08-24 nginx 默认版本升级到 nginx-1.31.4。

v1.52 2026-08-06 新增 SSL 证书管理和 Nginx 反向代理管理菜单。SSL 工具会扫描 /usr/local/nginx/ssl 下已部署的全部证书，支持续签检测和单域名强制重新签发；反向代理支持新增、修改、删除和列表操作。代理配置会透传任意静态资源，并将上游跳转地址和 Cookie 域重写为对外域名，用户访问时保持代理域名不变。

v1.51 2026-08-01 PHP版本更新8.5.9,8.4.24,8.3.33,8.2.33.并修复已失效的SSH隧道代理服务器.
